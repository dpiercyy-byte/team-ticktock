...
import { Archive, ArrowRight, FileSpreadsheet, Plus, RefreshCw, RotateCcw, Search, SlidersHorizontal } from "lucide-react";
...
function LeadsPage() {
  ...
  const syncing = useMutation({ mutationFn: async () => sync({ data: { token: token() } }), onSuccess: async (r) => { toast.success(`Imported ${r.imported} new lead${r.imported === 1 ? "" : "s"}`); await qc.invalidateQueries({ queryKey: ["meta-leads"] }); }, onError: (e: Error) => toast.error(e.message) });
  const move = useMutation({ mutationFn: async (input: { id: string; stage?: LeadStage; archived?: boolean; win?: boolean }) => input.win ? convert({ data: { token: token(), id: input.id } }) : update({ data: { token: token(), id: input.id, ...(input.stage ? { stage: input.stage } : {}), ...(input.archived === undefined ? {} : { archived: input.archived }) } }), onSuccess: async () => { await qc.invalidateQueries({ queryKey: ["meta-leads"] }); await qc.invalidateQueries({ queryKey: ["ledger", "jobs"] }); }, onError: (e: Error) => toast.error(e.message) });
  ...
  const visible = useMemo(() => (showArchived ? archived : active.filter((l) => l.stage === stage)).filter(matches), [active, archived, showArchived, stage, query]);
  const nextStage = (s: LeadStage) => LEAD_STAGES[LEAD_STAGES.indexOf(s) + 1];

  return <LedgerShell>
    <header className="mb-5 flex items-end justify-between gap-3">
      <div><p className="text-[13px] font-medium l-muted">{active.length} active lead{active.length === 1 ? "" : "s"}</p><h1 className="mt-1 display text-[34px] leading-[1.05] md:text-[42px]">Leads</h1></div>
    ...
    <div className="-mx-5 flex gap-2 overflow-x-auto px-5 pb-2 md:-mx-8 md:px-8">
      {LEAD_STAGES.map((s) => <Button key={s} size="sm" variant={!showArchived && stage === s ? "default" : "secondary"} onClick={() => { setShowArchived(false); setStage(s); }} className="shrink-0 rounded-full">{s} <span className="opacity-60">{active.filter((l) => l.stage === s).length}</span></Button>)}
      <Button size="sm" variant={showArchived ? "default" : "ghost"} onClick={() => setShowArchived((v) => !v)} className="shrink-0 rounded-full"><Archive/>Archived <span className="opacity-60">{archived.length}</span></Button>
    </div>
    ...
    {visible.length === 0 && <div className="l-card mt-4 px-6 py-14 text-center"><FileSpreadsheet className="mx-auto h-6 w-6 l-muted"/><p className="mt-3 text-sm l-muted">{showArchived ? "Nothing archived." : `No leads in ${stage}.`}</p></div>}
