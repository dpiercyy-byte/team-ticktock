...
export async function updateLead(id: string, patch: { stage?: LeadStage; assignedOwner?: string | null; nextAction?: string | null; nextActionDueAt?: string | null; qualificationStatus?: "qualified" | "needs_review" | "rejected"; lostReason?: string | null; archived?: boolean }) {
  ...
  const title = patch.archived === true ? "Lead archived" : patch.archived === false ? "Lead restored" : patch.stage ? `Moved to ${patch.stage}` : "Lead updated";
  const kind = patch.archived !== undefined ? "archive" : patch.stage ? "stage" : "update";
  await supabaseAdmin.from("lead_activities").insert({ lead_id: id, kind, title, detail: patch.lostReason ?? null }); return leadDto(data as Record<string, any>);
}
